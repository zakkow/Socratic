...
Each test uses its own course_id. `User.active` never resets to False on
its own (a student stays "active" until they unmatch or leave), so
reusing one course_id across tests would let a leftover active user
from an earlier test leak into a later test's candidate pool — that's
a test-isolation bug, not a production one, and it's exactly what
happened during initial development of this file.
"""
...
def test_block_is_bidirectional():
    """If A blocks B, B also shouldn't be matched to A — not just A->B."""
    course = fresh_course()
    alice, bob, _ = setup_two_matched_users(course)
    client.post("/users/block", json={"blocker_id": alice["id"], "blocked_id": bob["id"]})
    ...
    # Bob requests first this time (reversed order) to prove the
    # exclusion isn't accidentally direction-specific.
